...
public class ReedSolOperations {
    ...
    public static int[] bigVandermondeDistributionMatrix(int rows, int cols, int w) {
        ...
        for (i = 1; i < cols; i++) {
            ...
            if (j >= rows) {   /* This should never happen if rows/w are correct */
                System.err.println("reed_sol_big_vandermonde_distribution_matrix(" + rows + ", " + cols + "," + w +
                        ") - couldn't make matrix\n");
                System.exit(1);
            }
        ...
        for (i = cols + 1; i < rows; i++) {
            ...
            try {
                if (tmp != 1) {
                    tmp = Galois.singleDivide(1, tmp, w);
                    for (j = 0; j < cols; j++) {
                        dist[sindex + j] = Galois.singleMultiply(dist[sindex + j], tmp, w);
                    }
                }
                sindex += cols;
            } catch (Exception e) {
