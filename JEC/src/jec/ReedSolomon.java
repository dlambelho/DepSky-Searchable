...
public class ReedSolomon {
    ...
    private static Map<String, byte[]> doEncode(byte[] srcData, int k, int m, int w) {
        ...
        for (int i = 0; i < k && copied < srcData.length; i++) {
            int len = (i < k - 1 && srcData.length >= blocksize) ? blocksize :
                    ((srcData.length - (i * blocksize))) > 0 ? srcData.length - (i * blocksize) : 0;
    ...
    //================= DECODE ==================
    private static byte[] doDecode(Map<String, byte[]> srcData, int k, int m, int w, int originalSize)
            throws UnsuccessfulDecodeException {
